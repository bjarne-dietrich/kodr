...
func NewSSACRLNCEncoder(pieces []kodr_internals.Piece, sparsityLevel uint) *SSACRLNCEncoder {
	return &SSACRLNCEncoder{BaseEncoder: *base.NewBaseEncoder(pieces), sparsityLevel: sparsityLevel}
}
...
func NewSSACRLNCEncoderWithPieceCount(data []byte, pieceCount uint, sparsityLevel uint) (*SSACRLNCEncoder, error) {

	encoder, err := base.NewBaseEncoderWithPieceCount(data, pieceCount)
	if err != nil {
		return nil, err
	}
	return &SSACRLNCEncoder{BaseEncoder: *encoder, sparsityLevel: sparsityLevel}, nil
...
// appending zero-padding to data if needed.
func NewSSACRLNCEncoderWithPieceSize(data []byte, pieceSize uint, sparsityLevel uint) (*SSACRLNCEncoder, error) {
