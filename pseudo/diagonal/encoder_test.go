...
import (
	...
	"github.com/itzmeanjan/kodr/pseudo/diagonal"
)
...
func TestNewPseudoRLNC(t *testing.T) {
	t.Run("Encoder", func(t *testing.T) {
		...
		enc := diagonal.NewPseudoRLNCEncoder(pieces)
		dec := diagonal.NewPseudoRLNCDecoder(pieceCount)
	...
	t.Run("EncoderWithPieceCount", func(t *testing.T) {
		...
		enc, err := diagonal.NewPseudoRLNCEncoderWithPieceCount(data, pieceCount)
		if err != nil {
			t.Fatalf("Error: %s\n", err.Error())
		}
	...
	t.Run("EncoderWithPieceSize", func(t *testing.T) {
		...
		enc, err := diagonal.NewPseudoRLNCEncoderWithPieceSize(data, pieceSize)
		if err != nil {
			t.Fatalf("Error: %s\n", err.Error())
		}
...
func encoderFlow(t *testing.T, enc *diagonal.DiagonalPRLNCEncoder, dec *diagonal.DiagonalPRLNCDecoder, pieceCount uint, pieces []kodr_internals.Piece) {
	for {
