...
func (p *TrianglePRLNCDecoder) PieceLength() uint {
	if p.received > 0 {
...
func (p *TrianglePRLNCDecoder) IsDecoded() bool {
...
func (p *TrianglePRLNCDecoder) Required() uint {
...
func (p *TrianglePRLNCDecoder) GetPiece(i uint) (kodr_internals.Piece, error) {
	return p.state.GetPiece(i)
}

// All original pieces in order --- only when full decoding has happened
func (p *TrianglePRLNCDecoder) GetPieces() ([]kodr_internals.Piece, error) {
	if !p.IsDecoded() {
...
func NewTrianglePRLNCDecoder(pieceCount uint) *TrianglePRLNCDecoder {
	...
	return &TrianglePRLNCDecoder{expected: pieceCount, state: state}
}
